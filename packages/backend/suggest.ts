...
namespace rules {
  ...
  export function isRepeat(
    currentMeal: Meal,
    option: Meal,
    nextDay?: Meal | undefined
  ): boolean {
    return currentMeal.id === option.id;
  }

  /** returns true if double carbs */
  export function doubleCarbs(
    currentMeal: Meal,
    option: Meal,
    nextMealPlan?: Meal
  ): boolean {
    if (currentMeal.isHighCarb || nextMealPlan?.isHighCarb)
      return option.isHighCarb;
    else return false;
  }

  export function meetsLunchRules(
    previousDay: MealPlan,
    option: Meal
  ): boolean {
...
export async function getValidOptionsList(
  currentDay: MealPlan
): Promise<Meal[]> {
  ...
  if (futureDay !== undefined && futureDay.mealId !== null) {
    futureMeal = await findMealByID(futureDay.mealId);
  }
  ...
  const validOptions = allMeals.filter((item: Meal) => {
    if (rules.isRepeat(currentMeal, item, futureMeal)) return false;
      ...
      settings.carbRule &&
      rules.doubleCarbs(currentMeal, item, futureMeal)
    )
      return false;
  ...
  if (validOptions.length <= 0) return allMeals;
  return validOptions;
}
