...
export async function getAllMeals(): Promise<Meal[]> {
  return await prisma.meal.findMany({ include: { ingredients: true } });
}
...
export async function newMealPlan(
  isoDate: string,
  day: string,
  dinner: { mealName: any }
...
export async function getFutureDayPlan(
  input: string,
  n: number
): Promise<MealPlan | undefined> {
  const testDate = new Date(input);
  const futureDate = add(testDate, { days: n });
  const nextMealPlan = await prisma.mealPlan.findFirst({
    where: {
...
export async function newPlanWithDinner(
  date: string,
  day: string,
  dinner: Meal
) {
