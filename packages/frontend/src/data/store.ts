...
import {
  ...
  disconnectIngredientFromMeal,
  getIngredientsFromServer,
  getMealPlansFromServer,
  getMealsFromServer,
  getNextPlansFromServer,
  postNewMealToServer,
  postUpdateMealPlanToServer,
  postUpdateMealToServer,
} from "./helpers";
  ...
  actions: {
    ...
    async pushNewMeal(newMeal: ValidMeal) {
      try {
        ...
        if (confirmedMeal === undefined)
          throw Error("new meal returned undefined");
    ...
    async pushUpdatedMeal(updatedMeal: ValidMeal) {
      try {
        const oldIndex = this.meals.findIndex(
          (meal) => meal.id === updatedMeal.id
        );
        const ingsToRemove = this.oldIngredientsToRemove(
          this.meals[oldIndex].ingredients,
          updatedMeal.ingredients
        );
        const confirmedMeal = await postUpdateMealToServer(updatedMeal);
        if (confirmedMeal === undefined)
          throw Error("update meal returned undefined");
        if (ingsToRemove.length > 0) {
          ingsToRemove.map(
            async (item) =>
              await disconnectIngredientFromMeal(item, Number(updatedMeal.id))
          );
        }
        this.$patch((_state) => {
    ...
    async pushUpdatedMealPlan(updatedPlan: MealPlan) {
      if (updatedPlan.date) {
        updatedPlan.day = dayFromDate(new Date(updatedPlan.date));
      }
      const confirmedPlan = await postUpdateMealPlanToServer(updatedPlan);
      if (confirmedPlan === undefined)
        throw Error("updated plan returned undefined");
      const oldIndexCurr = this.mealPlans.findIndex(
        (plan) => plan.id === updatedPlan.id
      );
      if (oldIndexCurr >= 0) {
        this.mealPlans.splice(oldIndexCurr, 1, confirmedPlan);
      } else {
        const oldIndexNext = this.nextMealPlans.findIndex(
          (plan) => plan.id === updatedPlan.id
        );
    ...
    oldIngredientsToRemove(
      ...
      oldIngs.map((oldItem): void => {
        if (
          newIngs.some(
            (newItem) => newItem.ingredientName === oldItem.ingredientName
          )
        ) {
          return;
        } else {
          idsToRemove.push(oldItem);
        }
      });
    ...
    async removeIngredient(ingToRemove: Ingredient, meal: ValidMeal) {
      try {
        ...
        const newIngArray = meal.ingredients.filter(
          (ing) => ing !== ingToRemove
        );
        meal.ingredients = newIngArray;
        const oldIndex = this.meals.indexOf(meal);
        if (oldIndex === undefined)
          throw Error(`meal ${meal.mealName} missing from store`);
    ...
    findIngIdByName(ingName: string): Ingredient {
      const result = this.allIngredients.find(
        (ing) => ing.ingredientName === ingName
      );
      return result === undefined ? { ingredientName: ingName } : result;
    },

    findMealByID(idToFind: number): ValidMeal {
      const result = this.meals.find((meal) => meal.id === idToFind);
      if (result === undefined) return this.meals[0];
      return result;
    },
    ...
    pushDeleteIngredient(ingToDelete: Ingredient) {
      ...
      const newArray = this.allIngredients.filter(
